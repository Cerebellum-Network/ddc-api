...
**Updating proto definitions** (after new commits are pushed to `ddc-proto` master):

```bash
git submodule update --remote third_party/ddc-proto
cargo build
git add third_party/ddc-proto
git commit -m "Update ddc-proto submodule to latest"
```